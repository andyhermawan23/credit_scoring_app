...
# Kita buat kelas ini di level global agar terbaca oleh pickle
class _RemainderColsList(list):
    pass

# Daftarkan kelas ini ke dalam modul sklearn secara paksa
sklearn.compose._column_transformer._RemainderColsList = _RemainderColsList
# Masukkan juga ke namespace global script ini
sys.modules['__main__']._RemainderColsList = _RemainderColsList

# --- LANJUT KE LOAD MODEL ---
@st.cache_resource
def load_model():
    # Ganti dengan nama file .pkl Anda yang benar
    filename = 'best_model.pkl' 
    try:
        with open(filename, 'rb') as file:
            # Pickle sekarang akan mengenali _RemainderColsList sebagai list biasa
...
# --- PROSES PREDIKSI ---
# Definisikan list kolom sesuai urutan saat training (WAJIB SAMA)
feature_columns = [
    'tenor_months', 'loan_amount', 'monthly_installment', 'interest_rate_monthly_pct',
    'borrower_age', 'monthly_income', 'dti_ratio', 'credit_score', 'dpd',
    'outstanding_balance', 'par30_amount', 'par90_amount', 'par30_rate', 'par90_rate',
    'is_current', 'is_dpd1_29', 'is_dpd30', 'is_dpd60', 'is_dpd90', 'is_npl', 'is_wo',
    'max_dpd_cum', 'avg_dpd_cum', 'cnt_dpd1plus_cum', 'cnt_dpd30plus_cum',
    'cnt_dpd90plus_cum', 'ever_dpd30', 'ever_dpd90', 'max_dpd_3m', 'avg_dpd_3m',
    'cnt_dpd1plus_3m', 'dpd_worsened', 'dpd_improved', 'pct_tenor_elapsed',
    'pct_outstanding', 'product_type', 'channel', 'region', 'employment_type',
    'risk_grade', 'loan_purpose', 'loan_status'
]
...
if submit:
    ...
    # 1. Pastikan urutan kolom sesuai training (WAJIB)
    df_pred = df_pred[feature_columns] 
    
    # 2. Ambil probabilitas untuk kelas 1 (Default/Risk)
    # probability[0] adalah prob untuk kelas 0 (Lancar)
    # probability[1] adalah prob untuk kelas 1 (Default)
    prob_default = model.predict_proba(df_pred)[0][1]

    # 3. Logika Penentuan Kategori Risk sesuai Threshold Anda
    if prob_default > 0.50:
        risk_cat = "1. High Risk"
        risk_color = "error" # Merah
        risk_icon = "🚨"
    elif prob_default > 0.30:
        risk_cat = "2. Medium Risk"
        risk_color = "warning" # Kuning/Oranye
        risk_icon = "⚠️"
    else:
        risk_cat = "3. Low Risk"
        risk_color = "success" # Hijau
        risk_icon = "✅"
    ...
    with res_col1:
        st.markdown(f"### Result: {risk_icon} {risk_cat}")
        
        # Menampilkan pesan berdasarkan kategori
        if risk_color == "success":
            st.success("Aplikasi kredit disarankan untuk disetujui (Approved).")
        elif risk_color == "warning":
            st.warning("Perlu peninjauan manual lebih lanjut (Review Required).")
        else:
            st.error("Risiko gagal bayar terlalu tinggi (Rejected).")
    
    with res_col2:
        # Menampilkan Gauge/Metric Probabilitas
        st.metric(
            label="Default Probability", 
            value=f"{round(prob_default * 100, 2)}%",
            delta=risk_cat,
            delta_color="inverse" if prob_default > 0.3 else "normal"
        )

    # Opsional: Tambahkan visualisasi progress bar untuk level risiko
